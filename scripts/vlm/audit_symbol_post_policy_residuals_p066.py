#!/usr/bin/env python3
"""Audit residual symbol misses after the P0-65 optional complement policy."""
...
DEFAULT_POLICY = ROOT / "reports/vlm/symbol_rtdetr_complement_policy_p065_locked_predictions.jsonl"
...
DEFAULT_JSON = ROOT / "reports/vlm/symbol_post_policy_residuals_p066_locked.json"
DEFAULT_MD = ROOT / "reports/vlm/symbol_post_policy_residuals_p066_locked.md"
DEFAULT_EXAMPLES = ROOT / "reports/vlm/symbol_post_policy_residual_examples_p066_locked.jsonl"
...
def main() -> None:
    ...
    report = {
        "version": "symbol_post_policy_residuals_p066",
        "split": args.split,
    ...
    lines = [
        "# P0-66 post-policy residual audit",
        "",
        "## Summary",
        "",
        f"- gold: `{report['totals']['gold']}`",
        f"- policy IoU recall: `{report['totals']['policy_iou_recall']:.6f}`",
        f"- missed after policy: `{report['totals']['missed_after_policy']}` (`{report['totals']['miss_rate_after_policy']:.6f}`)",
        f"- solved by policy vs v28: `{report['totals']['solved_by_policy_vs_v28']}`",
