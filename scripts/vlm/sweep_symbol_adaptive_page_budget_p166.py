#!/usr/bin/env python3
"""P166 adaptive page-budget rescue over P160/P155A disagreement candidates."""
...
OUT_JSON = ROOT / "configs/vlm/symbol_adaptive_page_budget_p166.json"
OUT_MD = ROOT / "reports/vlm/symbol_adaptive_page_budget_p166.md"
OUT_OVERLAY = ROOT / "reports/vlm/symbol_policy_moe_overlay_p166_best.jsonl"
...
def adaptive_policies(diag: dict[str, Any]) -> list[dict[str, Any]]:
    useful_labels = list(diag.get("useful_label_counts", {}).keys())
    top3 = useful_labels[:3] or ["shower", "column", "stair"]
    top4 = useful_labels[:4] or top3 + ["equipment"]
    top6 = useful_labels[:6] or top4
    buckets = list(diag.get("useful_bucket_counts", {}).keys())[:3] or ["small", "tiny", "medium"]
    q = diag.get("score_quantiles", {})
    score_grid = sorted({0.2604, 0.2929, 0.34, float(q.get("q50", 0.2929))})
    label_sets = [top3, top4]
    bucket_sets = [buckets]
    budget_profiles = [
        {"name": "flat1", "default_budget": 1, "sparse_core_max": 8, "sparse_budget": 1, "dense_core_min": 40, "dense_budget": 1},
        {"name": "sparse2", "default_budget": 1, "sparse_core_max": 16, "sparse_budget": 2, "dense_core_min": 45, "dense_budget": 1},
        {"name": "mid2", "default_budget": 2, "sparse_core_max": 12, "sparse_budget": 2, "dense_core_min": 45, "dense_budget": 1},
    ]
    sort_modes = ["score"]
    policies = []
            ...
            for min_score in score_grid:
                for max_iou in [0.08, 0.15]:
                    for min_dist in [20.0, 28.0]:
                        for replace_mode in ["drop_lowest_global", "drop_lowest_same_label"]:
                            for profile in budget_profiles:
                                for sort_mode in sort_modes:
                                    name = (
                                        f"p166_{profile['name']}_l{len(labels)}_b{len(bucket_set)}"
                                        f"_s{min_score:.2f}_i{max_iou:.2f}_d{min_dist:.0f}"
                                        f"_{replace_mode}_{sort_mode}"
                                    )
                                    policy = {
                                        "name": name,
                                        "labels": labels,
                                        ...
                                        "sort_mode": sort_mode,
                                        "label_priority": {label: len(labels) - idx for idx, label in enumerate(labels)},
                                        **profile,
                                    }
                                    policies.append(policy)
    policies.append({
        "name": "p166_p165_equivalent",
        ...
        "append_nms_iou": 0.75,
        "replace_mode": "drop_lowest_global",
        "sort_mode": "score",
        "label_priority": {"shower": 3, "column": 2, "stair": 1},
        "name_profile": "p165_equivalent",
        "default_budget": 1,
        "sparse_core_max": 8,
        "sparse_budget": 1,
        "dense_core_min": 40,
...
def page_budget(core_count: int, policy: dict[str, Any]) -> int:
    if core_count <= policy["sparse_core_max"]:
        return int(policy["sparse_budget"])
    if core_count >= policy["dense_core_min"]:
        return int(policy["dense_budget"])
    return int(policy["default_budget"])


def selected_additions(core: list[dict[str, Any]], recall: list[dict[str, Any]], policy: dict[str, Any]) -> list[dict[str, Any]]:
    base = copy.deepcopy(policy)
    base["max_add_per_page"] = 999
    candidates = p165.select_additions(core, recall, base)
    if policy["sort_mode"] == "label_priority_score":
        priority = policy.get("label_priority", {})
        candidates.sort(key=lambda item: (priority.get(item["label"], 0), item["score"]), reverse=True)
    else:
        candidates.sort(key=lambda item: item["score"], reverse=True)
    budget = page_budget(len(core), policy)
...
def apply_policy(core_by_row: dict[str, list[dict[str, Any]]], recall_by_row: dict[str, list[dict[str, Any]]], policy: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    ...
    for row_id, core in core_by_row.items():
        additions = selected_additions(core, recall_by_row.get(row_id, []), policy)
        kept = p165.replace_core(core, additions, policy["replace_mode"])
        out[row_id] = kept + additions
    return out
...
def materialize(base_rows: list[dict[str, Any]], preds_by_row: dict[str, list[dict[str, Any]]], policy: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        row["symbol_policy_overlay"]["policy_id"] = "p166_best"
        row["symbol_policy_overlay"]["description"] = "P166 adaptive page-budget disagreement backfill over P160 core"
        row_id = str(row.get("row_id") or row.get("id"))
        for idx, item in enumerate(row.get("symbol_candidates") or []):
            item["id"] = f"{row_id}_p166_best_symbol_{idx:05d}"
            item["target_id"] = item["id"]
            item["source"] = "symbol_policy_overlay_p166_best"
            item.setdefault("metadata", {})["p166_policy"] = policy["name"]
...
def render_md(report: dict[str, Any]) -> str:
    lines = [
        "# P166 Symbol Adaptive Page Budget Rescue",
        "",
    ...
    lines.append(f"| `p166_best` | {best['precision']:.6f} | {best['recall']:.6f} | {best['f1']:.6f} | {best['center_recall']:.6f} | {best['prediction_inflation']:.6f} |")
    lines += [
        "",
        "## Best Policy",
        "",
...
def main() -> None:
    ...
    policies = adaptive_policies(diag)
    scored = []
    ...
    decision = "positive_adopt_p166" if best["metrics"]["f1"] > p165_metrics["f1"] else "negative_keep_p165"
    report = {
        "id": "SCI-P2-166-symbol-adaptive-page-budget-rescue",
        "created_on": "2026-05-17",
        "decision": decision,
        "claim_boundary": "Runtime-safe adaptive page-budget replacement/backfill over P160/P155A disagreement candidates; gold only used for offline evaluation.",
