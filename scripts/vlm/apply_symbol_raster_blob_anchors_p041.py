#!/usr/bin/env python3
"""P0-41: generate raster blob/connected-component anchors and score them with P0-37 head."""
...
def merge_topk(selected: dict[str, list[dict[str, Any]]], scored: dict[str, list[dict[str, Any]]], threshold: float, max_add_per_page: int, global_extra_budget: int, nms_iou: float) -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    proposals = []
    audit = Counter()
    for page_id, base_rows in selected.items():
        rows = []
        for item in sorted(scored.get(page_id, []), key=lambda row: float(row.get("proposal_head_score_p041") or 0.0), reverse=True):
            if float(item.get("proposal_head_score_p041") or 0.0) < threshold:
            ...
            if len(rows) >= max_add_per_page:
                break
        proposals.append((sum(float(row.get("proposal_head_score_p041") or 0.0) for row in rows), page_id, rows))
...
def main() -> None:
    ...
    parser.add_argument("--output", default="reports/vlm/symbol_raster_blob_anchors_p041_smoke_eval.json")
    ...
    out = {
        "version": "symbol_raster_blob_anchors_p041_page_eval",
        "split": args.split,
        ...
        "source_integrity": {
            ...
            "runtime_features": ["raster page pixels", "connected component/blob anchors", "proposal-head crop scoring"],
