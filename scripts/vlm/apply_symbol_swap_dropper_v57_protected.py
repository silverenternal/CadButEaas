#!/usr/bin/env python3
"""Apply v56 swap dropper with runtime-safe protected keep rules."""
...
from train_symbol_swap_dropper_v56 import is_drop_safe, vector
...
def protected_candidate_ids(selected: list[dict[str, Any]]) -> set[str]:
    protected: set[str] = set()
    by_cluster: dict[str, list[dict[str, Any]]] = {}
    for row in selected:
        by_cluster.setdefault(cluster_key(row), []).append(row)
    for rows in by_cluster.values():
        rows.sort(key=lambda row: (float(row.get("policy_score") or 0.0), float(row.get("score") or 0.0)), reverse=True)
        if rows:
            cid = str(rows[0].get("candidate_id") or "")
            if cid:
                protected.add(cid)
    return protected


def choose_protected_drops(
    dropper_pack: dict[str, Any],
    selected: list[dict[str, Any]],
    additions: list[dict[str, Any]],
    min_drop_score: float,
    min_policy_gap: float,
) -> tuple[list[dict[str, Any]], Counter]:
    audit = Counter()
    if not additions:
        return [], audit
    protected = protected_candidate_ids(selected)
    add_floor = min(float(row.get("policy_score") or 0.0) for row in additions)
    names = dropper_pack["feature_names"]
    model = dropper_pack["model"]
    candidates = []
    if selected:
        probs = model.predict_proba(np.asarray([vector(row, names) for row in selected], dtype=np.float32))[:, 1]
    else:
        probs = []
    for row, prob in zip(selected, probs, strict=True):
        cid = str(row.get("candidate_id") or "")
        if cid in protected:
            audit["protected_cluster_top1"] += 1
            continue
        if float(row.get("policy_score") or 0.0) > add_floor - min_policy_gap:
            audit["protected_policy_gap"] += 1
            continue
        item = dict(row)
        item["drop_safe_score"] = float(prob)
        if float(prob) >= min_drop_score:
            candidates.append(item)
    candidates.sort(key=lambda row: (float(row.get("drop_safe_score") or 0.0), -float(row.get("policy_score") or 0.0)), reverse=True)
    drops = candidates[:len(additions)]
...
def eval_policy(
    ...
    for page_id, page_rows in pages.items():
        ...
        drops, drop_audit = choose_protected_drops(dropper_pack, selected, additions, min_drop_score, min_policy_gap)
        audit.update(drop_audit)
        if len(drops) != len(additions):
            additions = []
            drops = []
    ...
    return baseline, metrics, audit, predictions
...
def main() -> None:
    ...
    parser.add_argument("--min-policy-gap", type=float, default=0.0)
    parser.add_argument("--output", default="reports/vlm/symbol_swap_dropper_v57_protected_all_cache_eval.json")
    parser.add_argument("--predictions-output", default="reports/vlm/symbol_swap_dropper_v57_protected_all_cache_predictions.jsonl")
    ...
    baseline, selected, audit, predictions = eval_policy(
        scored,
    ...
    report = {
        "version": "symbol_swap_dropper_v57_protected",
        "source_integrity": {
        ...
        "policy": {
            ...
            "min_policy_gap": args.min_policy_gap,
            "protected_rules": ["cluster_top1", "policy_gap"],
        },
        "baseline": baseline,
        "selected": selected,
