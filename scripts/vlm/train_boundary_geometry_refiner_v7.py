#!/usr/bin/env python3
"""Train and apply a boundary geometry refiner for v7 visual model output.
...
Unlike the v6 smoke run, v7 defaults to full train/dev and full locked splits.
Use --smoke when a fast integration check is intended.
"""
...
def main() -> None:
    ...
    parser.add_argument("--output-stream", default="reports/vlm/real_upstream_model_predictions_model_v7_boundary.jsonl")
    parser.add_argument("--checkpoint", default="checkpoints/boundary_geometry_refiner_v7/model.joblib")
    parser.add_argument("--eval", default="reports/vlm/boundary_geometry_refiner_v7_eval.json")
    parser.add_argument("--summary", default="checkpoints/boundary_geometry_refiner_v7/train_summary.json")
    parser.add_argument("--min-probability", type=float, default=0.55)
    parser.add_argument("--smoke", action="store_true", help="Run a small integration check. Smoke reports are never adoption evidence.")
    ...
    max_locked_records = args.max_locked_records
    if args.smoke:
        max_train_records = max_train_records or 120
        max_locked_records = max_locked_records or 120
    if max_train_records > 0:
        train_rows = train_rows[:max_train_records]
    if max_locked_records > 0:
        locked_rows = locked_rows[:max_locked_records]
    ...
    print(json.dumps({"phase": "collect", "mode": "smoke" if args.smoke else "full", "train_rows": len(train_rows), "locked_rows": len(locked_rows)}, ensure_ascii=False), flush=True)
    ...
    print(json.dumps({"phase": "fit", "train_count": len(train_items), "locked_count": len(locked_items)}, ensure_ascii=False), flush=True)
    ...
    full_run = not args.smoke and max_train_records == 0 and max_locked_records == 0
    adopted = full_run and metrics["macro_f1"] >= 0.995 and metrics["accuracy"] >= 0.995
    ...
    eval_report = {
        "version": "boundary_geometry_refiner_v7_eval",
        "trained": True,
        "run_mode": "full" if full_run else "smoke_or_limited",
        "adopted": adopted,
        ...
        "record_limits": {"smoke": args.smoke, "max_train_records": max_train_records, "max_locked_records": max_locked_records},
    ...
    write_json(args.summary, {"version": "boundary_geometry_refiner_v7_train_summary", **eval_report})
    ...
    for row in load_jsonl(input_path):
        ...
        for node in ((out.get("scene_graph") or {}).get("nodes") or []):
            ...
            if isinstance(metadata, dict):
                metadata["boundary_geometry_refiner_v7"] = {
                    "probability": round(prob, 6),
                    "model": "boundary_geometry_refiner_v7",
        ...
        out.setdefault("route_trace", {})["boundary_geometry_refiner_v7"] = {
