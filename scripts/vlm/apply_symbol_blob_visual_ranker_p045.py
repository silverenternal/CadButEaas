#!/usr/bin/env python3
"""P0-45: visual/context runtime-safe ranker for blob-anchor proposals."""
...
from train_symbol_blob_visual_ranker_p045 import runtime_vector
...
def score_anchors(
    ...
    for page_id in pages:
        ...
        for start in range(0, len(pending), args.batch_size):
            ...
            ranker_x = []
            typed = []
            rank_crop_size = int(ranker_bundle.get("rank_crop_size") or 16)
            for (idx, box, _features), score, type_idx in zip(batch, scores, type_indices, strict=True):
                ...
                typed.append((idx, box, float(score), label))
                ranker_x.append(runtime_vector(image, box, float(score), label, rank_crop_size))
            rank_scores = ranker_model.predict_proba(np.asarray(ranker_x, dtype=np.float32))[:, 1] if ranker_x else []
            for (idx, box, score, label), rank_score in zip(typed, rank_scores, strict=True):
                ...
                item = {
                    ...
                    "proposal_rank_score_p044": float(rank_score),
                    "proposal_rank_score_p045": float(rank_score),
                    "proposal_source": "p045_visual_blob_ranker",
...
def calibrated_score(row: dict[str, Any], label_boost: dict[str, float]) -> float:
    ...
    base = float(row.get("proposal_rank_score_p044", row.get("proposal_head_score_p041") or 0.0))
    return base + label_boost.get(label, 0.0)
...
def main() -> None:
    ...
    parser.add_argument("--ranker", default="checkpoints/symbol_blob_visual_ranker_p045/model.joblib")
    ...
    parser.add_argument("--output", default="reports/vlm/symbol_blob_visual_ranker_p045_smoke_eval.json")
    ...
    ranker_bundle = joblib.load(source_path(args.ranker))
    scored, score_audit = score_anchors(pages, selected, image_by_page, bundle, ranker_bundle, args)
    ...
    out = {
        "version": "symbol_blob_visual_ranker_p045_page_eval",
        "split": args.split,
        ...
        "source_integrity": {
            ...
            "runtime_features": ["raster page pixels", "connected component/blob anchors", "proposal-head crop scoring", "runtime-safe visual/context ranker", "runtime-only per-label calibration"],
