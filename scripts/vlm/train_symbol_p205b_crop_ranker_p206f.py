#!/usr/bin/env python3
"""P206f cached hard-mined crop ranker/regressor for P205b additions."""
    ...
    for rid, preds in pred_maps.items():
        ...
        for idx, pred in enumerate(preds):
            ...
            examples.append({
                ...
                "context_score": float(context_scores.get((rid, idx), 0.0)),
                "p200_real_prob": float((crop_scores.get((rid, idx), {}) or {}).get("p200_real_prob", 0.0)),
                "p200_fp_prob": float((crop_scores.get((rid, idx), {}) or {}).get("p200_false_positive_prob", 1.0)),
                "crop": crop_tensor(row, pred["bbox"], crop_size, pad_ratio),
            })
    return examples, dim


def cache_payload(examples: list[dict[str, Any]], dim: int, args: argparse.Namespace) -> dict[str, Any]:
    compact = []
    for ex in examples:
        item = {k: v for k, v in ex.items() if k != "crop"}
        item["crop"] = ex["crop"].to(torch.float16)
        compact.append(item)
    return {
        "id": "P206f_candidate_cache",
        "feature_dim": dim,
...
def save_cache(path: Path, examples: list[dict[str, Any]], dim: int, args: argparse.Namespace) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(cache_payload(examples, dim, args), path)


def load_cache(path: Path) -> tuple[list[dict[str, Any]], int]:
    payload = torch.load(path, map_location="cpu")
    examples = payload["examples"]
    for ex in examples:
        ex["crop"] = ex["crop"].float()
    return examples, int(payload["feature_dim"])


class CandidateDataset(Dataset):
    def __init__(self, examples: list[dict[str, Any]], ids: set[str], augment: bool = False) -> None:
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.augment:
            ...
            scale = 0.85 + random.random() * 0.30
            bias = (random.random() - 0.5) * 0.12
            crop = torch.clamp(crop * scale + bias, 0.0, 1.0)
            if random.random() < 0.35:
                crop = torch.clamp(crop + torch.randn_like(crop) * 0.025, 0.0, 1.0)
        return (
            crop,
...
def example_weight(ex: dict[str, Any], pos_base: float) -> float:
    label_bonus = 1.35 if ex.get("label") in {"sink", "shower", "equipment"} else 1.0
    bucket_bonus = 1.45 if ex.get("bucket") in {"tiny", "small"} else 1.0
    hard_positive = 1.0 + max(0.0, 0.45 - float(ex.get("p200_real_prob", 0.0))) + max(0.0, 0.35 - float(ex.get("context_score", 0.0)))
    hard_negative = 1.0 + max(0.0, float(ex.get("detector_score", 0.0)) - 0.10) * 2.0 + max(0.0, float(ex.get("p200_real_prob", 0.0)) - 0.45)
    if float(ex.get("target", 0.0)) >= 0.5:
        return pos_base * label_bonus * bucket_bonus * hard_positive
    return label_bonus * bucket_bonus * hard_negative


def focal_bce_loss(logits: torch.Tensor, targets: torch.Tensor, alpha: float, gamma: float) -> torch.Tensor:
...
def train_model(model: CropRankReg, train_ds: CandidateDataset, val_ds: CandidateDataset, args: argparse.Namespace, device: torch.device) -> dict[str, Any]:
    ...
    pos_base = neg / max(pos, 1.0)
    weights = torch.tensor([example_weight(ex, pos_base) for ex in train_ds.examples], dtype=torch.double)
    sampler = WeightedRandomSampler(weights, num_samples=int(len(train_ds) * args.samples_per_epoch), replacement=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for crop, feat, y, center, delta in train_loader:
            ...
            loss_rank = (1.0 - args.focal_mix) * rank_loss(logits, y) + args.focal_mix * focal_bce_loss(logits, y, args.focal_alpha, args.focal_gamma)
            mask = center[:, None]
    ...
    for row in base_rows:
        ...
        for idx, pred in enumerate(pred_maps[rid]):
            ...
            if policy.get("apply_regression"):
                ...
                raw["bbox"] = item["bbox"]
                raw.setdefault("metadata", {})["p206f_regression"] = {"rank_score": round(sc["rank_score"], 6), "delta": [round(v, 6) for v in sc["pred_delta"]]}
                item["raw"] = raw
            ...
            if best_iou >= policy["max_iou_to_core"] or best_dist < policy["min_dist_to_core"]:
                continue
            item["p206f_candidate_index"] = idx
            additions.append(item)
        additions.sort(key=lambda pred: scored[(rid, int(pred["p206f_candidate_index"]))]["rank_score"], reverse=True)
...
def materialize(rows: list[dict[str, Any]], pred_map: dict[str, list[dict[str, Any]]], policy: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        for idx, pred in enumerate(pred_map.get(rid, [])):
            ...
            item["id"] = f"{rid}_p206f_symbol_{idx:05d}"
            item["target_id"] = item["id"]
            item["source"] = "symbol_p205b_crop_ranker_p206f"
        ...
        new["symbol_policy_overlay"] = {"policy_id": "p206f_crop_ranker", "policy": policy}
        out.append(new)
    return out
...
def render(report: dict[str, Any]) -> str:
    ...
    e = report.get("p206e_metrics", d)
    m = report["best_metrics"]
    lines = [
        "# P206f Crop-Visual Candidate Ranker/Regressor",
        "",
        ...
        f"| `P206d` | {d['precision']:.6f} | {d['recall']:.6f} | {d['f1']:.6f} | {d['center_recall']:.6f} | {d['prediction_inflation']:.6f} |",
        f"| `P206e_current_best` | {e['precision']:.6f} | {e['recall']:.6f} | {e['f1']:.6f} | {e['center_recall']:.6f} | {e['prediction_inflation']:.6f} |",
        f"| `P206f_best` | {m['precision']:.6f} | {m['recall']:.6f} | {m['f1']:.6f} | {m['center_recall']:.6f} | {m['prediction_inflation']:.6f} |",
        "",
        "## Best Policy",
        "",
        "```json",
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", default="checkpoints/symbol_p205b_crop_ranker_p206f/model.pt")
    parser.add_argument("--out-json", default="configs/vlm/symbol_p205b_crop_ranker_p206f.json")
    parser.add_argument("--out-md", default="reports/vlm/symbol_p205b_crop_ranker_p206f_eval.md")
    parser.add_argument("--out-overlay", default="reports/vlm/symbol_p205b_crop_ranker_p206f_overlay.jsonl")
    ...
    parser.add_argument("--cache", default="datasets/symbol_p205b_candidate_cache_p206f/manifest.pt")
    parser.add_argument("--rebuild-cache", action="store_true")
    parser.add_argument("--samples-per-epoch", type=float, default=1.6)
    parser.add_argument("--focal-mix", type=float, default=0.45)
    ...
    if cache_path.exists() and not args.rebuild_cache:
        examples, dim = load_cache(cache_path)
        cache_status = "loaded"
    else:
        ...
        save_cache(cache_path, examples, dim, args)
        cache_status = "rebuilt"
    train_ids, val_ids, test_ids = split_ids(base_rows, args.seed)
    train_ds = CandidateDataset(examples, train_ids, augment=True)
    ...
    p206e_metrics = p165.evaluate(golds, {row_id(row): p165.normalized(row.get("symbol_candidates") or [], "p206e") for row in p206e_rows})
        ...
        for threshold in thresholds:
            for max_add in [0, 1, 2, 3, 5, 8]:
                for apply_regression in [False, True]:
                    policies.append({"name": f"p206f_l{len(labels)}_t{threshold:.2f}_a{max_add}_r{int(apply_regression)}", "labels": sorted(labels), "rank_threshold": threshold, "max_add_per_row": max_add, "max_iou_to_core": 0.08, "min_dist_to_core": 8, "apply_regression": apply_regression, "delta_clamp": 1.2})
    scored_policies = []
    ...
    report = {
        "id": "P206f_p205b_crop_ranker_regressor",
        ...
        "cache": {"path": args.cache, "status": cache_status},
        "training": training,
        ...
        "delta_vs_p206e": p165.delta(best_policy["metrics"], p206e_metrics),
        "decision": "promote_candidate" if best_policy["metrics"]["f1"] > p206e_metrics["f1"] else "no_promotion_keep_P206e",
    ...
    print(json.dumps({"decision": report["decision"], "candidate_counts": report["candidate_counts"], "cache": report["cache"], "baseline": baseline, "p206d": p206d_metrics, "p206e": p206e_metrics, "best_metrics": best_policy["metrics"], "delta_vs_p206e": report["delta_vs_p206e"], "outputs": report["outputs"]}, ensure_ascii=False, indent=2))
