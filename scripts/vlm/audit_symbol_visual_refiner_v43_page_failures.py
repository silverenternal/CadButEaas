#!/usr/bin/env python3
"""Audit page-level misses after the v43 hard-case visual refiner."""
...
def refined_ids_from_predictions(prediction_rows: list[dict[str, Any]]) -> set[str]:
    ...
    for page in prediction_rows:
        for pred in page.get("predicted_symbols") or []:
            if bool(pred.get("refined_by_v38")):
...
def candidate_context(
    ...
    for row in rows:
        ...
        if str(labels.get("best_iou_target_id") or labels.get("target_id") or "") == target_id or target_id in {
            str(v) for v in labels.get("center_target_ids") or []
        }:
            target_rows += 1
    ...
    if best_iou >= 0.30:
        reason = "candidate_available_but_page_matching_or_duplicate_issue"
    elif center_rows > 0 and target_rows > 0 and refined_target_rows == 0:
        reason = "hardcase_candidate_not_refined_by_v43_coverage_gap"
    elif center_rows > 0:
...
def audit(
    ...
    report = {
        "version": "symbol_visual_refiner_v43_page_failure_audit",
        "task": "P1-15D-v43-top-failure-audit",
        "claim_boundary": "Audits locked page-level gold symbols still missed after v43 routed visual bbox refinement.",
        "source_integrity": {
        ...
        "summary": {
            ...
            "note": "This is an attribution audit using any candidate hit, not the one-to-one page metric used in locked eval.",
        },
        ...
        "decision_hint": "If most misses are proposal_recall_gap_no_near_candidate, prioritize proposal expert. If center_candidate_exists_but_box_quality_still_low dominates, continue visual/refiner. If candidate_available_but_page_matching_or_duplicate_issue dominates, prioritize suppression/listwise matching.",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--predictions", default="reports/vlm/symbol_visual_box_refiner_v43_hardcases_page_locked_predictions.jsonl")
    parser.add_argument("--output", default="reports/vlm/symbol_visual_box_refiner_v43_hardcases_failure_audit.json")
    parser.add_argument("--examples-output", default="reports/vlm/symbol_visual_box_refiner_v43_hardcases_failure_examples.jsonl")
