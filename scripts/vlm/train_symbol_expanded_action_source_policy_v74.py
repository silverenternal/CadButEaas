#!/usr/bin/env python3
"""Train v74 expanded selector with selected-set conflict features.
...
def selected_context(row: dict[str, Any], selected: list[dict[str, Any]]) -> dict[str, float]:
    ...
    if row_box is None or not selected:
        return {
    ...
    for item in selected:
        ...
        if cluster_key(item) == row_cluster:
...
def vector(row: dict[str, Any], names: list[str]) -> list[float]:
    ...
    values: dict[str, float] = {
        ...
        "nearest_selected_iou": float(feats.get("nearest_selected_iou") or 0.0),
        "nearest_same_label_iou": float(feats.get("nearest_same_label_iou") or 0.0),
        "nearest_selected_center_dist": float(feats.get("nearest_selected_center_dist") or 0.0),
        "same_label_score_margin": float(feats.get("same_label_score_margin") or 0.0),
        "same_area_score_margin": float(feats.get("same_area_score_margin") or 0.0),
        "same_cluster_score_margin": float(feats.get("same_cluster_score_margin") or 0.0),
        "overlap_selected_count_iou_0_10": float(feats.get("overlap_selected_count_iou_0_10") or 0.0),
        "overlap_selected_count_iou_0_30": float(feats.get("overlap_selected_count_iou_0_30") or 0.0),
    }
    ...
    for page_id, selected in base.items():
        ...
        for action, score in sorted(actions_by_page.get(page_id, []), key=lambda pair: (pair[1], float(pair[0].get("rescue_score") or 0.0)), reverse=True):
            ...
            if candidate is None:
                ...
                continue
            item = dict(candidate)
            item["expanded_action_score_v74"] = score
            added.append(item)
            ...
            if len(added) >= max_add_per_page:
                break
        proposals.append((audit.get("added_bucket:new_iou_target", 0), sum(float(x.get("expanded_action_score_v74") or 0.0) for x in added), page_id, selected + added, audit))
    used_extra = 0
            ...
            "predicted_symbols": [
                {
                    ...
                    "bbox": row.get("bbox"),
                    "label": row.get("label"),
                    "confidence": round(float(row.get("expanded_action_score_v74", row.get("score") or 0.0)), 6),
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="checkpoints/symbol_expanded_action_source_policy_v74")
    parser.add_argument("--dataset-output", default="datasets/symbol_expanded_action_source_policy_v74/actions.jsonl")
    parser.add_argument("--dataset-manifest-output", default="datasets/symbol_expanded_action_source_policy_v74/manifest.json")
    parser.add_argument("--eval-output", default="reports/vlm/symbol_expanded_action_source_policy_v74_eval.json")
    parser.add_argument("--smoke-predictions-output", default="reports/vlm/symbol_expanded_action_source_policy_v74_smoke_predictions.jsonl")
    ...
    write_json(source_path(args.dataset_manifest_output), {
        "version": "symbol_expanded_action_source_policy_v74_dataset",
        ...
        "source_integrity": {
            "gold_used_for_inference": False,
            "runtime_uses_svg_or_cad_geometry": False,
            "runtime_features": ["score", "label", "bbox area", "cluster/page features", "cluster rank", "source reason derived from runtime metadata", "selected-set overlap and score-margin features"],
    ...
    report = {
        "version": "symbol_expanded_action_source_policy_v74",
        "source_integrity": {
            "gold_used_for_inference": False,
            "runtime_uses_svg_or_cad_geometry": False,
            "runtime_features": ["candidate score", "predicted label", "bbox area", "cluster/page features", "cluster rank", "source reason from runtime metadata", "selected-set overlap and score-margin features"],
