#!/usr/bin/env python3
"""Train a SymbolFixtureExpert v8 — ExtraTrees tabular model for 9-class symbol classification.

Key insight from prior attempts:
- v6 MLP (crop pixels): 7-class F1=0.61 — crop pixels too tiny for MLP
- v7 CNN (crop pixels): 7-class F1=0.35 — crops too small for CNN
- bbox-prototype baseline: 7-class F1=0.92 — bbox features are strongest signal

v8 strategy: Enhanced bbox + room context + neighbor features → ExtraTrees
Features (13 total):
  1-6: Bbox (cx, cy, w, h, area_norm, log_aspect_ratio)
  7-10: Room context (wet, living, service, outdoor)
  11-13: Neighbor (count, avg_area_log, area_ratio_log)

Target: 9-class dev macro F1 >= 0.90
"""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="checkpoints/symbol_fixture_expert_v8")
    ...
    # Extract features — limit to 30k for speed, stratified to include rare classes
    print("Extracting train features...")
    train_items = extract_items_stratified(train_rows, max_symbols=30000)
    ...
    print(f"\nTraining ExtraTrees (n={args.n_estimators}, depth={args.max_depth})...")
    ...
    summary = {
        "model_type": "symbol_fixture_expert_v8_extra_trees",
    ...
    for split_name, split_rows in splits.items():
        ...
        if split_name == "dev":
            items = extract_items_stratified(split_rows, max_symbols=15000)
        else:
            items = extract_items(split_rows, max_symbols=5000)
        if not items:
            continue
        ...
        y_pred = y_pred[valid]
        y_gold = y_gold[valid]
        items = [item for item, v in zip(items, valid) if v]

        predictions = build_predictions(items, y_gold, y_pred, labels, model, X_scaled)
    ...
    # Save model as joblib for runtime inference
    import joblib
    joblib.dump({
        "classifier": model,
        "encoder": None,  # labels are direct from label_to_index
        "scaler": scaler,
        "feature_names": FEATURE_NAMES,
        "class_names": labels,
        "label_to_index": label_to_index,
    }, output_dir / "model_v8.joblib")

    write_json(output_dir / "train_summary.json", summary)
    print(f"\nFinal: dev macro F1={dev_f1:.4f} {'PASS' if dev_f1 >= 0.90 else 'FAIL'} (target >= 0.90)")
...
def extract_items_stratified(rows: list[dict[str, Any]], max_symbols: int = 30000) -> list[dict[str, Any]]:
    ...
    # First pass: collect all symbols per row (but don't extract features yet)
    row_symbols = []
    for row in rows:
    ...
    # Keep all rare classes (< 100), subsample abundant ones
    kept = []
    rare_threshold = 100
    ...
    # Now extract features for kept symbols
    items = []
    # Group by row to compute neighbor features correctly
...
def extract_items(rows: list[dict[str, Any]], max_symbols: int = 0) -> list[dict[str, Any]]:
    items = []
    for row in rows:
        if max_symbols > 0 and len(items) >= max_symbols:
            break
        width = float((row.get("metadata") or {}).get("width") or 2000.0)
        height = float((row.get("metadata") or {}).get("height") or 2000.0)
...
def memory_audit(stage: str) -> dict:
