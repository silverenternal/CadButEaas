#!/usr/bin/env python3
"""P0-44: learned runtime-safe ranker for blob-anchor proposals."""
...
from train_symbol_blob_anchor_ranker_p044 import runtime_vector
    ...
    for page_id in pages:
        ...
        for start in range(0, len(pending), args.batch_size):
            ...
            for (idx, box, _features), score, type_idx in zip(batch, scores, type_indices, strict=True):
                ...
                ranker_x.append(runtime_vector(box, float(score), label))
            ...
            for (idx, box, score, label), rank_score in zip(typed, rank_scores, strict=True):
                ...
                item = {
                    ...
                    "proposal_source": "p044_learned_blob_ranker",
...
def main() -> None:
    ...
    parser.add_argument("--ranker", default="checkpoints/symbol_blob_anchor_ranker_p044/model.joblib")
    ...
    parser.add_argument("--output", default="reports/vlm/symbol_blob_learned_ranker_p044_smoke_eval.json")
    ...
    out = {
        "version": "symbol_blob_learned_ranker_p044_page_eval",
        "split": args.split,
        ...
        "source_integrity": {
            ...
            "runtime_features": ["raster page pixels", "connected component/blob anchors", "proposal-head crop scoring", "runtime-safe learned ranker", "runtime-only per-label calibration"],
