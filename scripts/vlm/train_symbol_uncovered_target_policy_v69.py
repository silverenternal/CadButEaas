#!/usr/bin/env python3
"""Train/apply v69 uncovered-target policy with selected-set conflict features."""
...
def evaluate_policy(
    ...
    for row, score in zip(split_actions, scores, strict=True):
        ...
        if float(score) < threshold:
            continue
        if float(feats.get("same_cluster_score_margin") or 0.0) < min_score_margin:
            route["filtered_low_same_cluster_margin"] += 1
            continue
        if float(feats.get("nearest_selected_iou") or 0.0) > max_nearest_iou:
    ...
    for page_id, selected in base.items():
        ...
        for action, score in sorted(actions_by_page.get(page_id, []), key=lambda pair: pair[1], reverse=True):
            ...
            bucket = str((action.get("labels") or {}).get("bucket") or "unknown")
            audit["added"] += 1
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="checkpoints/symbol_uncovered_target_policy_v69")
    parser.add_argument("--eval-output", default="reports/vlm/symbol_uncovered_target_policy_v69_eval.json")
    parser.add_argument("--smoke-predictions-output", default="reports/vlm/symbol_uncovered_target_policy_v69_smoke_predictions.jsonl")
    ...
    for threshold in [0.02, 0.05, 0.08, 0.12, 0.18]:
        for max_add_per_page in [5, 8, 10]:
            for min_score_margin in [-1.0, -0.5, -0.2]:
                for max_nearest_iou in [0.999, 0.7, 0.5]:
    ...
    smoke_metrics, smoke_selected, smoke_audit = evaluate_policy(
        ...
        args.candidate_inflation_target,
        float(selected["min_score_margin"]),
        float(selected["max_nearest_iou"]),
    )
    report = {
        "version": "symbol_uncovered_target_policy_v69",
        "source_integrity": {
        ...
        "selected_policy": {
            ...
            "min_score_margin": selected["min_score_margin"],
            "max_nearest_iou": selected["max_nearest_iou"],
            "selected_on": "dev",
        },
        ...
        "gate": {
            "smoke_recall_gt_v68": smoke_metrics["symbol_bbox_iou_0_30"]["recall"] > 0.685,
        ...
        "dev_grid_top": [
            {
                ...
                "max_nearest_iou": row["max_nearest_iou"],
