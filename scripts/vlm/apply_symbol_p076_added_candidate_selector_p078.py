#!/usr/bin/env python3
"""Apply fixed P0-78 P0-76 added-candidate selector gate."""
...
from train_symbol_p076_added_candidate_selector_p078 import apply_feature_gate
...
FIXED_CONFIG={'score_min':0.1,'score_max':1.01,'overlap_min':0.45,'overlap_max':1.01,'area_min':0.0,'area_max':1000000000000.0,'aspect_min':0.0,'aspect_max':1000000000000.0,'max_add_per_page':20}
...
def main():
 ...
 gated=apply_feature_gate(v28,p076,FIXED_CONFIG); metrics=score(golds,gated,{r:[] for r in gated},{'labels':'all','areas':'all','score_min':1.1,'max_iou_with_v28':0.0,'max_add_per_page':0})
 delta_v28={k:round(metrics[k]-baseline[k],6) for k in ['iou_0_30_recall','center_recall','candidate_inflation','precision']}; delta_p076={k:round(metrics[k]-full[k],6) for k in ['iou_0_30_recall','center_recall','candidate_inflation','precision']}
 retained=round((metrics['iou_0_30_recall']-baseline['iou_0_30_recall'])/max(full['iou_0_30_recall']-baseline['iou_0_30_recall'],1e-9),6)
 decision='positive_locked_selector_gate' if retained>=0.7 and metrics['precision']>=full['precision'] and metrics['candidate_inflation']<=full['candidate_inflation'] else 'negative_do_not_package'
 ...
 report={'version':'symbol_p076_selector_p078_fixed','split':args.split,'source_integrity':'runtime selector uses candidate score/bbox/overlap features; gold validation-only','inputs':{'v28_predictions':rel(Path(args.v28_predictions)),'p076_predictions':rel(Path(args.p076_predictions))},'outputs':{'predictions':rel(outp),'json':rel(outj),'markdown':rel(outm)},'gate_config':FIXED_CONFIG,'baseline_v28':baseline,'full_p076':full,'selected_policy':metrics,'delta_vs_v28':delta_v28,'delta_vs_p076':delta_p076,'retained_p076_gain_fraction':retained,'decision':decision}
 outj.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
 outm.write_text(f"""# P0-78 fixed P0-76 selector - {args.split}\n\n## Decision\n\n- `{decision}`\n\n## Metrics\n\n- v28 IoU / inflation / precision: `{baseline['iou_0_30_recall']:.6f}` / `{baseline['candidate_inflation']:.6f}` / `{baseline['precision']:.6f}`\n- P0-76 IoU / inflation / precision: `{full['iou_0_30_recall']:.6f}` / `{full['candidate_inflation']:.6f}` / `{full['precision']:.6f}`\n- selected IoU / inflation / precision: `{metrics['iou_0_30_recall']:.6f}` / `{metrics['candidate_inflation']:.6f}` / `{metrics['precision']:.6f}`\n- retained P0-76 gain: `{retained:.6f}`\n- delta vs P0-76 IoU / inflation / precision: `{delta_p076['iou_0_30_recall']:+.6f}` / `{delta_p076['candidate_inflation']:+.6f}` / `{delta_p076['precision']:+.6f}`\n""")
 print(json.dumps({'split':args.split,'decision':decision,'retained':retained,'delta_vs_p076':delta_p076,'selected':{k:metrics[k] for k in ['iou_0_30_recall','candidate_inflation','precision']}},ensure_ascii=False,indent=2))
