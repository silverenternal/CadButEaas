#!/usr/bin/env python3
"""Train a small crop+geometry MLP for TextDimensionExpert.

This is still a compact baseline, but unlike the dependency-free prototype it
uses raster crop statistics plus normalized geometry and records CUDA memory.
"""
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="checkpoints/cadstruct_moe_text_dimension_crop_mlp")
    parser.add_argument("--epochs", type=int, default=8)
    parser.add_argument("--batch-size", type=int, default=4096)
    parser.add_argument("--hidden-dim", type=int, default=96)
    parser.add_argument("--dropout", type=float, default=0.1)
    parser.add_argument("--lr", type=float, default=1e-3)
    parser.add_argument("--max-train-items", type=int, default=120000)
    ...
    model = TextMLP(len(FEATURE_NAMES), args.hidden_dim, len(labels), args.dropout).to(device)
    ...
    metadata = {
        "model_type": "text_dimension_crop_mlp",
        "labels": labels,
        ...
        "device": str(device),
        "notes": "Crop-statistics MLP baseline; replace with OCR-aware crop encoder for paper metrics.",
    }
...
def crop_stats(image: Image.Image | None, bbox: list[float]) -> list[float]:
    ...
    crop = image.crop((x1, y1, x2, y2)).resize((24, 24))
