#!/usr/bin/env python3
"""Train a raster-only symbol center+size proposal head.

This is the P0-01 follow-up after the v24 center-only heatmap failed.  It keeps
the same locked page metric, but predicts per-center width/height so tiny and
skinny symbols are not decoded with one fixed box size.
"""
...
from train_symbol_tile_detector_v20 import FORBIDDEN_RUNTIME_FIELDS, load_jsonl, rel, source_path, target_area_buckets, write_json, write_jsonl


ROOT = Path(__file__).resolve().parents[2]
DATA = ROOT / "datasets/symbol_tile_detector_tiny_sahi_v21"
CHECKPOINT = ROOT / "checkpoints/symbol_center_size_proposal_v25"
REPORT = ROOT / "reports/vlm/symbol_center_size_proposal_v25_eval.json"
PREDICTIONS = ROOT / "reports/vlm/symbol_center_size_proposal_v25_predictions.jsonl"


class CenterSizeDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, dict[str, Any]]:
        ...
        for target in ((row.get("targets") or {}).get("boxes") or []):
            ...
            bw = max(1.0, page_box[2] - page_box[0])
            bh = max(1.0, page_box[3] - page_box[1])
            ...
            size[:, gy, gx] = [np.log1p(bw), np.log1p(bh)]
...
class CenterSizeNet(nn.Module):
    def __init__(self) -> None:
        ...
        self.size = nn.Conv2d(128, 2, 1)
        nn.init.constant_(self.obj.bias, -5.0)

    def forward(self, images: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        feat = self.backbone(images)
        return self.obj(feat), self.size(feat)
...
def center_loss(logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    ...
    weights = torch.where(positive, torch.full_like(targets, 24.0), torch.full_like(targets, 1.0))
...
def train_epoch(model: nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer, device: torch.device) -> dict[str, float]:
    ...
    for images, heatmaps, sizes, size_masks, _metas in loader:
        ...
        denom = size_masks.sum().clamp_min(1.0)
        size_loss = (F.smooth_l1_loss(pred_sizes, sizes, reduction="none") * size_masks).sum() / denom
...
def decode(obj_logits: torch.Tensor, size_logits: torch.Tensor, metas: list[dict[str, Any]], input_size: int, stride: int, score_threshold: float, topk: int, min_box: float, max_box: float) -> dict[str, list[dict[str, Any]]]:
    probs = torch.sigmoid(obj_logits).detach().cpu()
    sizes = torch.expm1(size_logits.detach().cpu()).clamp(min=min_box, max=max_box)
    ...
    for item in range(batch):
        ...
        for score, flat_index in zip(scores.tolist(), indices.tolist(), strict=True):
            ...
            bw = float(sizes[item, 0, gy, gx])
            bh = float(sizes[item, 1, gy, gx])
            page_preds[str(meta["row_id"])].append(
                {
                    ...
                    "score": float(score),
                    "tile_id": meta["id"],
                    "proposal_source": "center_size_v25",
                }
            )
    return page_preds
...
def collect(model: nn.Module, rows: list[dict[str, Any]], device: torch.device, args: argparse.Namespace) -> tuple[dict[str, list[dict[str, Any]]], dict[str, dict[str, dict[str, Any]]]]:
    dataset = CenterSizeDataset(rows, args.input_size, args.stride, augment=False, seed=args.seed)
    ...
    with torch.no_grad():
        for images, _heatmaps, _sizes, _size_masks, metas in loader:
            ...
            batch_preds = decode(obj_logits, size_logits, metas, args.input_size, args.stride, args.decode_score_threshold, args.topk_per_tile, args.min_box_size, args.max_box_size)
...
def main() -> None:
    ...
    parser.add_argument("--score-threshold-grid", default="0.003,0.005,0.01,0.02,0.03")
    ...
    parser.add_argument("--max-box-size", type=float, default=120.0)
    parser.add_argument("--union-center-size-score-scale", type=float, default=0.25)
    ...
    loader = DataLoader(
        CenterSizeDataset(train_rows, args.input_size, args.stride, augment=True, seed=args.seed),
    ...
    if Path(args.yolo_baseline_report).exists() and baseline_predictions_path.exists():
        ...
        union_preds = union_predictions(load_page_predictions(baseline_predictions_path), locked_preds, args.union_center_size_score_scale)
    ...
    write_json(
        ...
        {
            "model_type": "symbol_center_size_proposal_v25",
    ...
    report = {
        "version": "symbol_center_size_proposal_v25_eval",
        "claim_boundary": "Raster-only class-agnostic center+size proposal head; symbol typing remains downstream.",
        "source_integrity": {
        ...
        "locked_center_size": locked_eval,
        "locked_yolo_baseline": yolo_locked,
        "locked_yolo_center_size_union": union_eval,
    ...
    report["decision"] = "promote_center_size_route" if report["stage_gate"]["passed"] else "failed_gate_continue_instance_mask_or_stronger_center_size"
    write_json(Path(args.eval_output), report)
    write_jsonl(Path(args.predictions_output), locked_predictions)
    write_json(Path(args.eval_output).with_name("symbol_center_size_proposal_v25_error_buckets.json"), locked_errors[:2000])
    print(json.dumps({"locked_center_size": locked_eval, "locked_union": union_eval, "gate": report["stage_gate"], "decision": report["decision"]}, ensure_ascii=False, indent=2))
