#!/usr/bin/env python3
"""Small sink refresh rescue after P296.

P297 keeps P296 as the base experiment-line stream, trains a train-only
binary sink model, and uses dev to validate one predeclared threshold.
Locked is final audit only. Fine relation threshold is an internal locked
audit, not external validation.
"""
...
from evaluate_symbol_bathtub_refresh_rescue_p296 import (  # noqa: E402
    ADJUSTED_PREDICTIONS as BASE_P296_PREDICTIONS,
    FINE_SCORER_REPORT as P296_SCORER,
    MODEL_JOBLIB as P296_MODEL_JOBLIB,
    SELECTED_THRESHOLD as P296_BATHTUB_THRESHOLD,
    apply_bathtub_refresh,
    p295_labels_and_confidence,
)
...
TARGET_LABEL = "sink"
SELECTED_MODEL_NAME = "hgb_420_l003_leaf31_s20260526"
SELECTED_THRESHOLD = 0.70
THRESHOLDS = [0.65, 0.70, 0.75, 0.80, 0.85]
PROTECT_CURRENT_LABELS = {"generic_symbol", "bathtub", "shower"}

REPORT_JSON = ROOT / "reports" / "vlm" / "p297_symbol_sink_refresh_rescue_experiment.json"
REPORT_MD = ROOT / "reports" / "vlm" / "p297_symbol_sink_refresh_rescue_experiment.md"
POLICY_JSON = ROOT / "checkpoints" / "symbol_sink_refresh_rescue_p297" / "policy.json"
MODEL_JOBLIB = ROOT / "checkpoints" / "symbol_sink_refresh_rescue_p297" / "model.joblib"
ADJUSTED_PREDICTIONS = ROOT / "reports" / "vlm" / "real_upstream_predictions_dev_symbol_sink_refresh_rescue_p297.jsonl"
FUSION_REPORT = ROOT / "reports" / "vlm" / "symbol_sink_refresh_rescue_p297_eval.json"
SCORER_REPORT = ROOT / "reports" / "vlm" / "scene_graph_fusion_symbol_sink_refresh_rescue_p297_no_repair_scorer_v1_eval.json"
SCORER_DECISION = ROOT / "reports" / "vlm" / "relation_scorer_symbol_sink_refresh_rescue_p297_adoption_v1.json"
FINE_SCORER_REPORT = ROOT / "reports" / "vlm" / "scene_graph_fusion_symbol_sink_refresh_rescue_p297_fine_relation_no_repair_scorer_v1_eval.json"
FINE_DECISION_REPORT = ROOT / "reports" / "vlm" / "relation_scorer_symbol_sink_refresh_rescue_p297_fine_adoption_v1.json"
...
def p296_labels_and_confidence(
    data: dict[str, Any], split: str
) -> tuple[list[str], list[float], dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    labels, confidence, p291_application, column_application, shower_application = p295_labels_and_confidence(data, split)
    checkpoint = joblib.load(P296_MODEL_JOBLIB)
    bathtub_model = checkpoint["model"]
    bathtub_probability = positive_probability(bathtub_model, data["features"][split])
    labels, confidence, bathtub_application = apply_bathtub_refresh(
        labels,
        confidence,
        bathtub_probability,
        P296_BATHTUB_THRESHOLD,
    )
    return labels, confidence, p291_application, column_application, shower_application, bathtub_application


def apply_sink_refresh(
    labels: list[str],
    ...
    for row_index, current_label in enumerate(labels):
        if current_label in PROTECT_CURRENT_LABELS:
            source["protected"] += 1
            continue
        if float(sink_probability[row_index]) < threshold:
        ...
        out_confidence[row_index] = float(sink_probability[row_index])
    ...
    for prediction in base_predictions:
        ...
        if str(row.get("family")) == "symbol":
            ...
            if old_label != new_label:
                ...
                row["source"] = "symbol_sink_refresh_rescue_p297"
                metadata = dict(row.get("metadata") or {})
                metadata["symbol_sink_refresh_rescue_p297"] = {
                    "policy": policy,
...
def threshold_candidates(
    y_dev: list[str],
    p296_dev_labels: list[str],
    p296_dev_confidence: list[float],
    dev_probability: np.ndarray,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    baseline = metrics(y_dev, p296_dev_labels)
    rows = []
    for threshold in THRESHOLDS:
        labels, _confidence, application = apply_sink_refresh(p296_dev_labels, p296_dev_confidence, dev_probability, threshold)
        row_metrics = metrics(y_dev, labels)
        deltas = {
            "macro_f1_delta_pp": round((macro_f1(row_metrics) - macro_f1(baseline)) * 100.0, 4),
            "sink_f1_delta_pp": round((per_label_f1(row_metrics, TARGET_LABEL) - per_label_f1(baseline, TARGET_LABEL)) * 100.0, 4),
            ...
            "equipment_f1_delta_pp": round((per_label_f1(row_metrics, "equipment") - per_label_f1(baseline, "equipment")) * 100.0, 4),
            ...
            "appliance_f1_delta_pp": round((per_label_f1(row_metrics, "appliance") - per_label_f1(baseline, "appliance")) * 100.0, 4),
        }
        deltas["max_non_target_regression_pp"] = round(
            max(0.0, *[-value for key, value in deltas.items() if key.endswith("_f1_delta_pp") and key != "sink_f1_delta_pp"]),
            4,
        )
        rows.append(
            {
...
def select_threshold(dev_candidates: list[dict[str, Any]]) -> dict[str, Any]:
    eligible = [
        ...
        and row["dev_delta_vs_p296"]["macro_f1_delta_pp"] > 0.0
        and row["dev_delta_vs_p296"]["sink_f1_delta_pp"] > 0.0
        and row["dev_delta_vs_p296"]["generic_symbol_f1_delta_pp"] >= 0.0
        and row["dev_delta_vs_p296"]["bathtub_f1_delta_pp"] >= 0.0
        and row["dev_delta_vs_p296"]["shower_f1_delta_pp"] >= 0.0
        and row["dev_delta_vs_p296"]["max_non_target_regression_pp"] <= 0.05
        and int(row["application"]["changed_count"]) <= 30
    ]
    if not eligible:
        raise RuntimeError(f"predeclared threshold {SELECTED_THRESHOLD} is not dev-positive on top of P296")
    return eligible[0]
...
def run_fine_relation_scorer(predictions: list[dict[str, Any]], records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    p296 = load_json(P296_SCORER)
    scorer = {
        "version": "scene_graph_fusion_symbol_sink_refresh_rescue_p297_fine_relation_no_repair_scorer_v1",
        "created": "2026-05-26",
        ...
        "baseline_p296": {
            "source": str(P296_SCORER.relative_to(ROOT)),
            "node_macro_f1": (p296.get("node_evaluation") or {}).get("macro_f1"),
            "relation_f1": (p296.get("relation_evaluation") or {}).get("f1"),
        },
    }
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    delta_p296 = report["fine_relation_delta_vs_p296"]
    per = report["locked_symbol_metrics"]["per_label"]
    lines = [
        "# P297 Sink Refresh Rescue",
        "",
        "## Summary",
        f"- Selected model/threshold: `{report['selected_model_name']}` / `{report['selected_threshold']}`.",
        f"- Node macro-F1: `{fine['node_evaluation']['macro_f1']:.6f}` ({delta_p296['node_macro_f1_delta_pp']:+.4f} pp vs P296).",
        f"- Fine relation F1: `{fine['relation_evaluation']['f1']:.6f}` ({delta_p296['relation_f1_delta_pp']:+.4f} pp vs P296).",
        f"- sink/generic_symbol/bathtub/shower F1: `{per['sink']['f1']:.6f}` / `{per['generic_symbol']['f1']:.6f}` / `{per['bathtub']['f1']:.6f}` / `{per['shower']['f1']:.6f}`.",
        f"- Changed locked symbols: `{report['locked_application']['changed_count']}`.",
        f"- Status: `{report['status']}`.",
        "",
        "## Claim Boundary",
        "- Train split fits the binary sink refresh model.",
        "- Dev split validates the predeclared threshold with protected labels.",
...
def main() -> int:
    data = load_features()
    p296_dev_labels, p296_dev_confidence, p291_dev_application, column_dev_application, shower_dev_application, bathtub_dev_application = p296_labels_and_confidence(data, "dev")
    p296_locked_labels, p296_locked_confidence, p291_locked_application, column_locked_application, shower_locked_application, bathtub_locked_application = p296_labels_and_confidence(data, "locked")
    ...
    dev_baseline, candidates = threshold_candidates(data["labels"]["dev"], p296_dev_labels, p296_dev_confidence, dev_probability)
    selected = select_threshold(candidates)

    locked_labels, locked_confidence, locked_application = apply_sink_refresh(
        p296_locked_labels,
        p296_locked_confidence,
        locked_probability,
    ...
    joblib.dump(
        {
            ...
            "feature_policy": "p297_raw_44d_symbol_features_train_only_sink_binary",
            "selection_policy": "predeclared threshold 0.70 must be dev-positive on top of P296",
            "created": "2026-05-26",
        },
        MODEL_JOBLIB,
    )
    policy = {
        ...
        "base_policy": "P296 bathtub refresh rescue on top of P295",
        "selection_policy": "predeclared threshold 0.70; dev-positive gate; locked final audit only",
        "selected_dev_candidate": selected,
    }

    base_predictions = load_jsonl(BASE_P296_PREDICTIONS)
    adjusted = apply_to_predictions_p297(base_predictions, data["items"]["locked"], locked_labels, locked_confidence, locked_application, policy)
    write_jsonl(ADJUSTED_PREDICTIONS, adjusted)
    fusion = evaluate_fusion(adjusted, data["rows"]["locked"])
    fusion["version"] = "symbol_sink_refresh_rescue_p297_eval"
    ...
    coarse_delta_p296 = compact_delta(p296, coarse_scorer)
    fine_delta_p296 = compact_delta(p296, fine_scorer)
    status = (
        "passed_sink_refresh_node_and_fine_relation_candidate"
        if fine_delta_p296["node_macro_f1_delta_pp"] > 0.0
        and fine_delta_p296["relation_f1_delta_pp"] > 0.0
        and locked_metrics["per_label"]["generic_symbol"]["f1"] >= (p296.get("node_evaluation", {}).get("per_label", {}).get("generic_symbol", {}).get("f1") or 0.0)
        and locked_metrics["per_label"]["bathtub"]["f1"] >= (p296.get("node_evaluation", {}).get("per_label", {}).get("bathtub", {}).get("f1") or 0.0)
        and locked_metrics["per_label"]["shower"]["f1"] >= (p296.get("node_evaluation", {}).get("per_label", {}).get("shower", {}).get("f1") or 0.0)
        and fine_delta_p296["invalid_graph_rate"] == 0.0
        else "completed_node_or_relation_tradeoff_keep_p296"
    )
    decision = {
        "version": "relation_scorer_symbol_sink_refresh_rescue_p297_fine_adoption_v1",
        "created": "2026-05-26",
        "source": str(FINE_SCORER_REPORT.relative_to(ROOT)),
        "baseline_source": str(P296_SCORER.relative_to(ROOT)),
        "delta_vs_p296": fine_delta_p296,
        "status": status,
        "boundary": "Locked fine-threshold audit; do not present as external validation.",
    }
    report = {
        "version": "p297_symbol_sink_refresh_rescue_experiment",
        "created": "2026-05-26",
        "protocol": "Train-only HGB sink binary model on 44D symbol features; dev validates predeclared threshold 0.70 on top of P296; locked evaluates once; fine relation threshold is an internal locked audit.",
        "claim_boundary": "SVG/contract normalized-candidate symbol classification; not raster detector performance. P297 fine relation is a locked threshold audit, not external validation.",
        ...
        "p296_model_checkpoint": str(P296_MODEL_JOBLIB.relative_to(ROOT)),
        "model_checkpoint": str(MODEL_JOBLIB.relative_to(ROOT)),
        "base_predictions": str(BASE_P296_PREDICTIONS.relative_to(ROOT)),
        ...
        "dev_baseline_p296_symbol_metrics": dev_baseline,
        ...
        "bathtub_dev_application": bathtub_dev_application,
        "bathtub_locked_application": bathtub_locked_application,
        ...
        "coarse_scorer_delta_vs_p296": coarse_delta_p296,
        "fine_relation_scorer": fine_scorer,
        "fine_relation_delta_vs_p296": fine_delta_p296,
        ...
        json.dumps(
            {
                ...
                "selected_model_name": SELECTED_MODEL_NAME,
                "selected_threshold": float(selected["threshold"]),
                "fine_delta_vs_p296": fine_delta_p296,
                "locked_application": locked_application,
