...
DEFAULT_TRAIN = ROOT / "reports/vlm/floorplancad_line_json_primitive_cache_windowed_2048_s1536_v3_r2/train_windowed_primitive_cache.jsonl"
DEFAULT_VAL = ROOT / "reports/vlm/floorplancad_line_json_primitive_cache_windowed_2048_s1536_v3_r2/val_windowed_primitive_cache.jsonl"
...
def greedy_assignment_gpu_tensor(torch: Any, cost: Any) -> tuple[Any, Any]:
    ...
    Unlike the audited diagnostic variant, this never transfers costs to SciPy.
    Exact Hungarian remains mandatory for validation and final evaluation.
    """
...
def match_component_queries(
    ...
    query_labels = torch.full((num_queries,), IGNORE_LABEL, dtype=torch.long, device=device)
    ...
    with torch.no_grad():
        ...
        if thing_query_count is not None:
            ...
            stuff_targets = (target_labels >= 30) & (target_labels <= 34)
        ...
        if typed_stuff_slots:
            ...
            thing_targets = torch.nonzero((target_labels < 30) | (target_labels > 34), as_tuple=False).flatten()
            ...
            for label in STUFF_LABELS:
                ...
                if targets.numel() == 1:
                    assignments.append((thing_query_count + label - 30, int(targets.item())))
            if matching == "greedy_gpu_train":
                ...
                for label in STUFF_LABELS:
                    targets = torch.nonzero(target_labels == label, as_tuple=False).flatten()
                    if targets.numel() == 1:
                        rows_tensor = torch.cat([rows_tensor, torch.tensor([thing_query_count + label - 30], device=device)])
...
def rq_sq_quality_deployment_scores(
    ...
    if int(query_logits.shape[-1]) <= IGNORE_LABEL:
        raise ValueError("query_logits must include foreground and no-object labels")
    foreground_probability = (
        query_logits.float().softmax(dim=-1)[..., :IGNORE_LABEL].max(dim=-1).values.detach()
    )
