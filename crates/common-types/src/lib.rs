...
pub use robust_geometry::{
    incircle, orient2d, orient3d, ExactF64, Orientation, Orientation3D,
    PrecisionLevel as GeoPrecisionLevel,
};
pub use scene::*;
...
pub use relative_coords::{
    RelativeClosedLoop, RelativePoint, RelativeRawEdge, RelativeSceneState, SceneOrigin,
};
// 先导出 error，再显式导出 response 中的非冲突类型
pub use error::{
    CadError, DxfParseReason, ErrorCode, ErrorLocation, GeometryConstructionReason,
    GraphBuildReason, InternalErrorReason, IoErrorReason, LoopExtractReason, PdfParseReason,
...
pub use request::{
    ...
    RequestMetadata, ValidateGeometryRequest, VectorizePdfRequest,
};
// 显式导出 response 中的类型，避免与 error 中的 ValidationIssue/Severity 冲突
pub use circuit_breaker::*;
pub use response::{Response, ResponseStatus, ResponseTimer, ServiceError};
