...
use opencv::core::{set_num_threads, get_num_threads};
...
pub fn init_opencv_runtime(config: &OpenCvThreadConfig) -> Result<i32, String> {
    if let Some(threads) = config.threads {
        set_num_threads(threads)
            .map_err(|e| format!("设置 OpenCV 线程数失败：{}", e))?;
        
        let actual_threads = get_num_threads()
            .map_err(|e| format!("获取 OpenCV 线程数失败：{}", e))?;
    ...
    match opencv::core::get_version_string() {
        Ok(version) => {
            ...
            if version.starts_with("4.6.0") {
                warnings.push(
                    "OpenCV 4.6.0 在某些系统上可能有稳定性问题，建议升级到 4.8+".to_string()
                );
            }
        }
        Err(_) => {
