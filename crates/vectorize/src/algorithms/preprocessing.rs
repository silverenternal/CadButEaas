...
pub fn median_filter(image: &GrayImage, kernel_size: u32) -> GrayImage {
    ...
    // Small image fallback to avoid parallelization overhead
    if (width < 100) || (height < 100) {
        return median_filter_serial(image, kernel_size, half);
    }
...
fn median_filter_serial(image: &GrayImage, kernel_size: u32, half: u32) -> GrayImage {
    let (width, height) = image.dimensions();
...
pub fn gaussian_blur(image: &GrayImage, sigma: f32) -> GrayImage {
    ...
    if (width < 100) || (height < 100) {
        return gaussian_blur_serial(image, sigma);
    }
    ...
    result_pixels
        ...
        .for_each(|(y_idx, row)| {
            ...
            for x in 0..width {
                ...
                row[x as usize] = pixel_val as u8;
            }
        });
...
fn gaussian_blur_serial(image: &GrayImage, sigma: f32) -> GrayImage {
    let (width, height) = image.dimensions();
...
pub fn non_local_means(image: &GrayImage, h: f32) -> GrayImage {
    ...
    // Small image fallback to avoid parallelization overhead
    if (width < 100) || (height < 100) {
        return non_local_means_serial(image, h);
    }
    ...
    result_pixels
        ...
        .for_each(|(y_idx, row)| {
            ...
            for x in 0..width {
                ...
                if weight_total > 0.0 {
                    row[x as usize] = (weighted_sum / weight_total) as u8;
                } else {
                    row[x as usize] = center_val as u8;
                }
            }
        });
...
fn non_local_means_serial(image: &GrayImage, h: f32) -> GrayImage {
    let (width, height) = image.dimensions();
...
pub fn clahe(image: &GrayImage, clip_limit: f32, tile_grid_size: u32) -> GrayImage {
    ...
    // 小图像 fallback
    if (width < 100) || (height < 100) {
        return clahe_serial(image, clip_limit, tile_grid_size);
    }
    ...
    let tile_cdfs: Vec<[u8; 256]> = tile_indices
        .par_iter()
        .map(|&(tx, ty)| {
    ...
    result_pixels
        ...
        .for_each(|(y_idx, row)| {
            ...
            for x in 0..width {
                ...
                let cdf00 = &tile_cdfs[(tile_y * tiles_x + tile_x) as usize];
                row[x as usize] = cdf00[val];
            }
        });
...
fn clahe_serial(image: &GrayImage, clip_limit: f32, tile_grid_size: u32) -> GrayImage {
    let (width, height) = image.dimensions();
    let mut result = GrayImage::new(width, height);
