...
fn test_vectorize_all_pdf_files() {
    ...
    let testpdf_dir = PathBuf::from(&manifest_dir).join("../../testpdf");
    ...
    println!(
        "成功：{} ({:.1}%)",
        success_count,
        success_count as f64 / pdf_files.len() as f64 * 100.0
    );
    println!(
        "警告：{} ({:.1}%)",
        warning_count,
        warning_count as f64 / pdf_files.len() as f64 * 100.0
    );
    println!(
        "失败：{} ({:.1}%)",
        fail_count,
        fail_count as f64 / pdf_files.len() as f64 * 100.0
    );
...
fn test_single_pdf(
    ...
    // 读取 PDF 文件
    let pdf_content = std::fs::read(pdf_path).map_err(|e| format!("读取 PDF 失败：{}", e))?;
...
fn test_pdf_vectorize_quality_assessment() {
    ...
    println!(
        ...
        total_files,
        if total_files > 0 {
            clear_success as f64 / total_files as f64 * 100.0
        } else {
            0.0
        }
    );
    println!(
        ...
        total_files,
        if total_files > 0 {
            minor_issues as f64 / total_files as f64 * 100.0
        } else {
            0.0
        }
    );
    println!(
        ...
        total_files,
        if total_files > 0 {
            complex_failed as f64 / total_files as f64 * 100.0
        } else {
            0.0
        }
    );
