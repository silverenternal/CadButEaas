...
fn benchmark_topo_small_scale() {
    ...
    for size in sizes {
        ...
        println!("{:>10} | {:>10.2} μs | {:>10.2} μs",
            size, elapsed.as_micros() as f64, per_item);
    }
}
...
fn benchmark_topo_large_scale() {
    ...
    println!("\n=== Topo 大规模性能测试 ===");
    println!("{:>10} | {:>12} | {:>12} | {:>10}", "Size", "Time", "Per item", "Loops");
    println!("{:-<55}", "");
    ...
    for size in sizes {
        ...
        match result {
            Ok(r) => {
                ...
                println!("{:>10} | {:>10.3} ms | {:>10.3} ms | {:>10}",
                    size, elapsed.as_secs_f64(), per_item, r.all_loops.len());
            }
            Err(e) => {
...
fn benchmark_topo_extreme_scale() {
    ...
    for size in sizes {
        ...
        match result {
            Ok(_) => {
                let per_item = elapsed.as_secs_f64() * 1000.0 / size as f64;
                println!("{:>10} | {:>10.3} s | {:>10.3} ms",
                    size, elapsed.as_secs_f64(), per_item);
            }
            Err(e) => {
                println!("{:>10} | 失败：{:?}", size, e);
            }
...
fn benchmark_complexity_quick() {
    ...
    for i in 1..times.len() {
        ...
        println!("{} -> {}: 时间比率={:.2}x (期望 O(n log n) ≈ 2.1-2.3x)",
...
fn benchmark_complexity_full() {
    ...
    for i in 0..sizes.len() {
        ...
        println!("{:>8} | {:>8.3} ms | {:>10} | {:>10} | {:>10} | {:>8.2}x",
            sizes[i], times[i] * 1000.0, points, edges, loops, ratio);
    }
...
fn benchmark_memory_quick() {
    ...
    println!("\n=== 内存效率 (n={}) ===", size);
    println!("输入：{} bytes, 输出：{} bytes, 比率：{:.2}",
        input_size, output_size, output_size as f64 / input_size as f64);
...
fn benchmark_memory_full() {
    ...
    println!("\n=== 完整内存效率测试 ===");
    println!("{:>10} | {:>12} | {:>12} | {:>10}", "Size", "Input", "Output", "Ratio");
    println!("{:-<55}", "");
...
fn benchmark_100k_detailed() {
    ...
    match result {
        Ok(r) => {
            ...
            println!("处理时间：{:.3} ms ({:.2} s)", elapsed.as_secs_f64(), elapsed.as_secs_f64());
            ...
            let estimated_memory_mb = (r.points.len() * std::mem::size_of::<Point2>() 
                + r.edges.len() * std::mem::size_of::<(usize, usize)>()) as f64 / 1024.0 / 1024.0;
            println!("估算内存使用：{:.2} MB", estimated_memory_mb);
            
            // 断言：100K 线段处理时间应 < 10 秒（宽松限制）
            assert!(elapsed.as_secs() < 10, "100K 线段处理时间过长，可能需要优化");
...
fn benchmark_large_scale_comparison() {
    ...
    for size in &sizes {
        ...
        match result {
            Ok(r) => {
                ...
                println!("{:>10} | {:>8.3} ms | {:>8.3} ms | {:>10} | {:>10} | {:>8.2}",
                    size, elapsed.as_secs_f64() * 1000.0, per_line, r.points.len(), r.edges.len(), mem_mb);
            }
            Err(e) => {
                panic!("{} 线段处理失败：{:?}", size, e);
            }
    ...
    for i in 1..times.len() {
        ...
        println!("{} → {}: 时间比={:.2}x, 期望 O(n log n)={:.2}x, 实际/期望={:.2}",
            sizes[i - 1], sizes[i], time_ratio, expected_ratio, time_ratio / expected_ratio);
    }
...
fn benchmark_real_concert_hall() {
    ...
    let dxfs_dir = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap().parent().unwrap()
        .join("dxfs");
    ...
    for (file_name, expected_entities) in &concert_hall_files {
        ...
        match result {
            Ok(r) => {
                println!("{:>20} | {:>10} | {:>8.2} ms | {:>10} | {:>8}",
                    file_name, expected_entities, elapsed.as_secs_f64() * 1000.0, 
                    r.points.len(), r.edges.len());
            }
            Err(e) => {
                println!("{:>20} | {:>10} | {:>10} | {:>10} | {:>8}",
        ...
        match result {
            Ok(r) => {
                ...
                let memory_kb = (r.points.len() * std::mem::size_of::<Point2>()
                    + r.edges.len() * std::mem::size_of::<(usize, usize)>()) as f64 / 1024.0;
                
                let loop_count = r.all_loops.len();
                
                println!("{:>15} | {:>8} | {:>8.2} ms | {:>8.2} KB | {:>10}",
                    scenario, line_count, topo_time.as_secs_f64() * 1000.0,
                    memory_kb, loop_count);
            }
            Err(e) => {
                println!("{:>15} | {:>8} | {:>10} | {:>10} | {:>10}",
    ...
    for size in &sizes {
        ...
        match result {
            Ok(r) => {
                ...
                println!("{:>10} | {:>10} | {:>10} | {:>10.2} | {:>12.2}",
                    size, r.points.len(), r.edges.len(), memory_kb, bytes_per_line);
            }
            Err(e) => {
                println!("{:>10} | {:>10} | {:>10} | {:>10} | {:>12}",
                    size, "-", "-", "ERROR", "-");
