...
fn test_color_filter_red_only() {
    ...
    assert!(all_entities.len() > 0, "DXF 文件应该有实体");
    ...
    assert!(entities.len() <= all_entities.len(), 
        "过滤后实体数量 {} 不应该超过原始数量 {}", entities.len(), all_entities.len());

    println!("✓ 颜色过滤后实体数量：{} (从 {} 减少，减少比例：{:.1}%)", 
        entities.len(), all_entities.len(), (1.0 - reduction_ratio) * 100.0);
}
...
fn test_color_filter_multiple_colors() {
    ...
    if red_count + black_count > 0 {
        println!("  └─ 找到红色实体：{}, 黑色实体：{}, ByLayer: {}, 块定义颜色：{}", 
            red_count, black_count, bylayer_count, block_def_count);
    } else if entities.len() > 0 {
        println!("  └─ 所有实体都是 ByLayer/ByBlock 或来自块定义（保留原始颜色）");
    }

    println!("✓ 多颜色过滤后实体数量：{} (红色：{}, 黑色：{}, ByLayer: {}, 块定义颜色：{})",
        entities.len(), red_count, black_count, bylayer_count, block_def_count);
}
...
fn test_no_filter() {
    ...
    if red_count + bylayer_count > 0 {
        println!("  └─ 找到红色实体：{}, ByLayer: {}, 其他颜色 (块定义): {}", 
            red_count, bylayer_count, other_count);
    } else if filtered_entities.len() > 0 {
        println!("  └─ 所有实体都来自块定义（保留原始颜色）");
    }

    // 验证 3: 计算过滤减少比例
    let reduction_ratio = if all_entities.len() > 0 {
    ...
    println!("✓ 颜色过滤后实体数量：{} (从 {} 减少，减少比例：{:.1}%)",
        filtered_entities.len(), all_entities.len(), reduction_ratio);
}
...
fn test_lineweight_filter() {
    ...
    for entity in &entities {
        ...
        if let Some(lw) = lineweight_mm {
            ...
            assert!(
                lw >= 0.25 && lw <= 0.50,
                "过滤后的实体线宽应该在 0.25mm-0.50mm 范围内，但找到 {}mm", lw
            );
        }
    }
    ...
    // 打印统计信息
    println!("✓ 线宽过滤后实体数量：{} (从 {} 减少，减少比例：{:.1}%)",
        entities.len(), all_entities.len(), reduction_ratio);
