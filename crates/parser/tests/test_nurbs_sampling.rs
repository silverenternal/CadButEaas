...
fn test_nurbs_arc_discretization_accuracy() {
    ...
    for n in sample_counts {
        ...
        if n >= 200 {
            assert!(max_error < 0.5, "采样数 200 时误差应小于 0.5, 实际：{}", max_error);
        }
    }
...
fn test_chordal_error_calculation() {
    ...
    tracing::debug!("直线误差：{:.10}, 圆弧误差：{:.6}", 
        calculate_chordal_error(&line), calculate_chordal_error(&arc));
}
...
fn test_nurbs_segment_bounds() {
    ...
    tracing::debug!("段数估算：3 点->{}, 10 点->{}, 100 点->{}, 1000 点->{}", 
        estimate_segments(3), estimate_segments(10), 
        estimate_segments(100), estimate_segments(1000));
}
