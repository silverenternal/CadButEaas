...
use parser::{PdfParser, ParserService, ParseResult};
...
fn test_pdf_all_test_files() {
    ...
    for entry in std::fs::read_dir(&pdf_dir).expect("无法读取目录") {
...
fn test_pdf_parser_direct_flate() {
    ...
    // 应该成功解析（即使内容为空）
    assert!(result.is_ok(), "带 FlateDecode 的 PDF 应该能解析：{:?}", result.err());
