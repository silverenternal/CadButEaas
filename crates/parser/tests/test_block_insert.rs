...
fn test_block_definition_parsing() {
    ...
    // 3 把椅子 = 15 个实体
    assert!(entities.len() >= 15, "应该至少有 15 个实体（3 把椅子 × 5 个几何元素），实际：{}", entities.len());
...
fn test_block_transformation() {
    ...
    let parser = DxfParser::new();
    let (entities, _) = parser.parse_file_with_report(&temp_path).expect("解析应该成功");
...
fn test_block_layer_preservation() {
    ...
    for entity in &entities {
        ...
        assert_eq!(layer, Some("FURNITURE"), "实体应该来自 FURNITURE 图层，实际：{:?}", layer);
    }
