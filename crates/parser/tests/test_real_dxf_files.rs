...
mod common;
use common::{get_dxf_dir, find_first_parseable_dxf};
...
fn test_parse_all_real_dxf_files() {
    ...
    // 统计结果
    let success_count = results.iter().filter(|r| matches!(r.1, Ok(_))).count();
    let fail_count = results.iter().filter(|r| matches!(r.1, Err(_))).count();
...
fn test_entity_type_distribution() {
    ...
    let entities = parser.parse_file(&file_path)
        .expect(format!("{:?} 应该能解析成功", file_path).as_str());
    ...
    // 至少应该有一些实体
    assert!(entities.len() > 0, "example.dxf 应该包含实体");
}
...
fn test_layer_distribution() {
    ...
    let entities = parser.parse_file(&file_path)
        .expect(format!("解析失败：{:?}", file_path).as_str());
...
fn test_layer_filter() {
    ...
    // 先解析所有实体
    let all_entities = parser.parse_file(&file_path)
        .expect("解析失败");

    // 获取所有图层
    let mut layers: Vec<String> = all_entities.iter()
    ...
    let filtered_entities = parser_filtered.parse_file(&file_path)
        .expect("解析失败");
    ...
    println!("  过滤后实体数 (目标图层 '{}'): {}", filtered_entities.len(), target_layer);
    println!("  实际图层分布:");
...
fn test_binary_dxf_parsing() {
    use dxf::{Drawing, entities::{Entity, Line}};
    
    // 创建一个简单的 Drawing
    let mut drawing = Drawing::new();
    drawing.add_entity(Entity::new(
        dxf::entities::EntityType::Line(Line {
            p1: dxf::Point { x: 0.0, y: 0.0, z: 0.0 },
            p2: dxf::Point { x: 100.0, y: 100.0, z: 0.0 },
            ..Default::default()
        })
    ));
    ...
    drawing.save_binary(&mut buffer)
        .expect("保存二进制 DXF 失败");
    
    // 解析二进制 DXF
    let parser = DxfParser::new();
    let entities = parser.parse_bytes(&buffer)
        .expect("二进制 DXF 解析失败");
...
fn test_get_layer_list() {
    ...
    let layers = parser.get_layer_list(&file_path)
        .expect("获取图层列表失败");
...
fn test_parse_with_report() {
    ...
    let (entities, report) = parser.parse_file_with_report(&file_path)
        .expect("解析失败");
    ...
    assert!(!report.entity_type_distribution.is_empty(), "应该至少有一种实体类型");
}
...
fn test_smart_layer_detection() {
    ...
    // 获取所有图层
    let all_layers = parser.get_layer_list(&file_path)
        .expect("获取图层列表失败");
    ...
    // 测试墙体图层识别
    let wall_layers = parser.detect_wall_layers(&file_path)
        .expect("获取墙体图层失败");
    println!("\n墙体图层：{:?}", wall_layers);

    // 测试门窗图层识别
    let door_window_layers = parser.detect_door_window_layers(&file_path)
        .expect("获取门窗图层失败");
    println!("门窗图层：{:?}", door_window_layers);

    // 测试家具图层识别
    let furniture_layers = parser.detect_furniture_layers(&file_path)
        .expect("获取家具图层失败");
    println!("家具图层：{:?}", furniture_layers);

    // 测试标注图层识别
    let dimension_layers = parser.detect_dimension_layers(&file_path)
        .expect("获取标注图层失败");
    println!("标注图层：{:?}", dimension_layers);

    // 至少应该能识别出一些图层
    let total_detected = wall_layers.len() + door_window_layers.len()
        + furniture_layers.len() + dimension_layers.len();
    ...
    for layer in &wall_layers {
        assert!(all_layers.contains(layer), "墙体图层 '{}' 不在原始图层列表中", layer);
    }
    for layer in &door_window_layers {
        assert!(all_layers.contains(layer), "门窗图层 '{}' 不在原始图层列表中", layer);
    }
    for layer in &furniture_layers {
        assert!(all_layers.contains(layer), "家具图层 '{}' 不在原始图层列表中", layer);
    }
    for layer in &dimension_layers {
        assert!(all_layers.contains(layer), "标注图层 '{}' 不在原始图层列表中", layer);
    }
}
...
fn test_unit_parsing() {
    ...
    let (_, report) = parser.parse_file_with_report(&file_path)
        .expect("解析失败");

    println!("\n图纸单位：{:?}, 比例因子：{}", report.drawing_units, report.unit_scale);
        ...
        for entity in &entities {
            let coords: Vec<f64> = match entity {
                ...
                common_types::RawEntity::Arc { center, radius, .. } => {
                    vec![center[0] - radius, center[0] + radius, 
        ...
        if let Some(units) = &report.drawing_units {
            ...
            if unit_str.contains("米") || unit_str.contains("meter") {
                ...
                if max_coord > 10000.0 {
                    assert!(report.unit_mismatch_detected, 
...
fn test_problem_file_endpoint_mismatch() {
    ...
    // 文件应该存在
    if !file_path.exists() {
        panic!("问题文件不存在：{:?}\n请检查 dxfs/ 目录是否包含该测试文件", file_path);
    }
    ...
    for entity in &entities {
        if let common_types::RawEntity::Line { end, metadata, .. } = entity {
            ...
            if end[0] == 0.0 && end[1] == 100.3 {
                ...
                assert!(mismatch > 0.1, "应该检测到端点错位，但错位太小：{}mm", mismatch);
