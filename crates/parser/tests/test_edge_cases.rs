...
use parser::{ParserService, FileType, ParseResult};
...
fn test_performance_many_entities() {
    ...
    let result = service.parse_bytes(&dxf_content, FileType::Dxf);
    let elapsed = start.elapsed();
    
    match &result {
        Ok(parse_result) => {
            let count = get_entities(parse_result);
            tracing::info!("1000 实体 DXF 解析成功：{} 实体，耗时：{:?}", count, elapsed);
            assert_eq!(count, 1000);
            assert!(elapsed.as_millis() < 100, "解析 1000 实体耗时过长：{:?}", elapsed);
        }
        Err(e) => {
            panic!("解析失败：{:?}", e);
        }
...
fn test_concurrent_parsing() {
    ...
    for i in 0..4 {
        ...
        let handle = thread::spawn(move || {
            let service = ParserService::new();
            let dxf = format!("0\nSECTION\n2\nENTITIES\n0\nLINE\n10\n{}.0\n20\n0.0\n30\n0.0\n11\n{}.0\n21\n10.0\n31\n0.0\n0\nENDSEC\n0\nEOF\n", i_copy, i_copy + 5);
            let result = service.parse_bytes(dxf.as_bytes(), FileType::Dxf);
            assert!(result.is_ok());
            let count = get_entities(&result.unwrap());
            count
        });
        handles.push(handle);
    }
