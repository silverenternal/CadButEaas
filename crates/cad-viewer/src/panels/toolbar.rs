...
use crate::components::{OpenFileCommand, ExportSceneCommand, AutoTraceCommand, DetectGapsCommand, ClearSelectionCommand, ToggleLassoToolCommand, UndoCommand, RedoCommand};
    ...
    fn render(&mut self, ctx: &egui::Context, comp_ctx: &mut ComponentContext) {
        ...
        egui::TopBottomPanel::top("toolbar")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    // 标题使用强调色
                    ui.label(egui::RichText::new("🛠️ CAD Viewer")
                        .strong()
                        .color(theme.accent)
                        .size(15.0));
                    ui.separator();
                    ...
                    // 撤销按钮
                    if ui.add_enabled(can_undo, egui::Button::new("↩ 撤销"))
                        .on_hover_text(format!("撤销 (Ctrl+Z)\n剩余 {} 步", comp_ctx.command_manager.undo_depth()))
                        .clicked() {
                        comp_ctx.push_command(UndoCommand);
                    }

                    // 重做按钮
                    if ui.add_enabled(can_redo, egui::Button::new("↪ 重做"))
                        .on_hover_text(format!("重做 (Ctrl+Y)\n剩余 {} 步", comp_ctx.command_manager.redo_depth()))
                        .clicked() {
                    ...
                    #[cfg(feature = "gpu")]
                    {
                        ...
                        let btn_text = if effects_enabled { "🎨 视觉效果 ✓" } else { "🎨 视觉效果" };
                        ...
                        ui.label(egui::RichText::new(format!("GPU: {}", gpu_tier))
                    ...
                    if let Some(path) = &comp_ctx.state.scene.file_path {
                        ui.separator();
                        ui.label(egui::RichText::new(format!("📄 {}", path.display().to_string())).color(theme.text_secondary));
                    }
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        ui.label(egui::RichText::new(ws_status)
                            .color(ws_color)
                            .size(11.0))
