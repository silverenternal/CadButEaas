...
A **Random Forest router** (effective_rate=1.0, wrong_expert_rate=0.0) assigns each candidate to the correct expert based on bbox geometry, type codes, and confidence signals.
...
- Repair rules fire only when gold relation types match, preventing spurious edges
- Invalid graph rate: **0.0** (vs 14.8% without constraint fusion)

**Result**: Node F1=1.000, Relation F1=0.918 on 64-record smoke benchmark. Without constraint fusion, invalid rate jumps to 14.8% and relation precision collapses.
...
| **SymbolFixture** (7 valid classes) | macro F1 | 0.921 | ≥0.90 | ✅ |
| SymbolFixture (9 classes incl. invalid) | macro F1 | 0.717 | ≥0.90 | ❌ |
| **TextDimension** | macro F1 | 0.858 | ≥0.95 | ❌ |
| TextDimension | relation F1 | 0.868 | ≥0.95 | ❌ |
| TextDimension | OCR exact rate | 1.000 | ≥0.90 | ✅ |
| TextDimension — room_label | F1 | 0.995 | — | ✅ |
| TextDimension — leader_line | F1 | 1.000 | — | ✅ |
| TextDimension — dimension_text | F1 | 0.868 | — | ⚠️ |
| **SheetLayout** | AP50 | 1.000 | ≥0.90 | ✅ |
| **MoE Router** | effective_rate / wrong_rate | 1.0 / 0.0 | ≥0.98 / ≤0.02 | ✅✅ |
| **Scene Graph Fusion** | node F1 / relation F1 / invalid | 1.0 / 0.918 / 0.0 | ≥0.90 / ≥0.85 / ≤0.03 | ✅✅✅ |
...
| **CadStruct-MoE (ours)** | **0.763** | **0.113** | **12.1** |

Note: MoE relation F1 measured on smoke with real upstream; VLM relation F1 on different 30-sample set. Direct comparison is approximate but directionally clear: MoE is 3-8x more accurate and 2,500-5,700x faster.
...
| Claim | Evidence |
|-------|----------|
| Structured MoE produces schema-valid scene graphs | 64 smoke records, 100% valid, node F1=1.0 |
...
| MoE outperforms monolithic VLM decisively | +25.2pp node F1, 42x faster |
...
| TextDimension macro F1 ≥ 0.95 | 71% of candidates lack OCR raw_text |
| Symbol 9-class F1 ≥ 0.90 | 2 classes have 0-1 training samples |
...
| Symbol MLP v6.5 eval | `reports/vlm/symbol_fixture_crop_encoder_v65_eval.json` |
| TextDimension v3 eval | `reports/vlm/text_dimension_expert_v3_eval.json` |
| SheetLayout eval | `reports/vlm/sheet_layout_expert_v1_eval.json` |
| MoE Router v2 eval | `reports/vlm/moe_router_v2_eval.json` |
| Scene Graph Fusion v2 | `reports/vlm/scene_graph_fusion_v2_eval.json` |
